...
pub(crate) type NormalizeChunkFn = fn(&[u8], &mut [u8], bool, bool) -> Result<NormalizeChunkStatus>;

pub(crate) type NormalizeFn = fn(&[u8]) -> Vec<u8>;

pub(crate) fn normalize(input: &[u8], fn_normalize_chunk: NormalizeChunkFn) -> Vec<u8> {
...
pub(crate) fn normalize_str(input: &str, fn_normalize: NormalizeFn) -> String {
...
pub trait Spec {
    const FN_NORMALIZE_CHUNK: NormalizeChunkFn;
}
