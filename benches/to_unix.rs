...
fn bench_to_unix_newlines(c: &mut Criterion) {
    ...
    assert_eq!(input.to_unix_newlines(), UNIX_REGEX.replace_all(&input, "\n"));
    ...
    assert_eq!(large_input.to_unix_newlines(), UNIX_REGEX.replace_all(&large_input, "\n"));
    ...
    c.bench_function("3rd party crate \"newline_normalizer\": to_unix_newlines()", |b| {
        let input_slice = input.as_str();
        b.iter(|| newline_normalizer::ToUnixNewlines::to_unix_newlines(black_box(input_slice)))
    });

    c.bench_function("3rd party crate \"newline_normalizer\": to_unix_newlines() with pre-normalized text", |b| {
        let input_slice = pre_normalized_input.as_str();
        b.iter(|| newline_normalizer::ToUnixNewlines::to_unix_newlines(black_box(input_slice)))
    });

    c.bench_function("3rd party crate \"newline_normalizer\": to_unix_newlines() with large ASCII text", |b| {
    ...
    c.bench_function("this crate: LF::normalize_str() with pre-normalized text", |b| {
        let input_slice = pre_normalized_input.as_str();
        b.iter(|| LF::normalize_str(black_box(input_slice)))
    });

    c.bench_function("this crate: LF::normalize_str() with large ASCII text", |b| {
        let input_slice = large_input.as_str();
        b.iter(|| LF::normalize_str(black_box(input_slice)))
    });

    c.bench_function("this crate: LF::normalize_str() with pre-normalized large ASCII text", |b| {
        let input_slice = pre_normalized_large_input.as_str();
        b.iter(|| LF::normalize_str(black_box(input_slice)))
    });
}
