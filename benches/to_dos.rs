...
fn bench_to_unix_newlines(c: &mut Criterion) {
    ...
    c.bench_function("3rd-party crate \"newline_normalizer\": to_dos_newlines()", |b| {
        let input_slice = input.as_str();
        b.iter(|| newline_normalizer::ToDosNewlines::to_dos_newlines(black_box(input_slice)))
    });

    c.bench_function("3rd-party crate \"newline_normalizer\": to_dos_newlines() with pre-normalized text", |b| {
        let input_slice = pre_normalized_input.as_str();
        b.iter(|| newline_normalizer::ToDosNewlines::to_dos_newlines(black_box(input_slice)))
    });

    c.bench_function("3rd-party crate \"newline_normalizer\": to_dos_newlines() for large input", |b| {
        let input_slice = large_input.as_str();
        b.iter(|| newline_normalizer::ToDosNewlines::to_dos_newlines(black_box(input_slice)))
    });
    ...
    c.bench_function("this crate: CRLF::normalize_str() with pre-normalized text", |b| {
        let input_slice = pre_normalized_input.as_str();
        b.iter(|| CRLF::normalize_str(black_box(input_slice)))
    });
    ...
    c.bench_function("this crate: CRLF::normalize_str() for large input with pre-normalized text", |b| {
        let input_slice = pre_normalized_large_input.as_str();
        b.iter(|| CRLF::normalize_str(black_box(input_slice)))
    });
}
