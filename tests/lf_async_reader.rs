...
dual_test!(crlf_split_across_readers, {
    ...
    let mut nr = LF::wrap_async_reader_with_buffer_size(test_reader, 3);
    let mut out = Vec::new();
...
dual_test!(lone_lf_in_first_reader_kept_as_lf, {
...
dual_test!(multiple_crs_and_crlf_mixed_across_boundaries, {
    ...
    let test_reader = AsyncTestReader::new(readers);
    let mut nr = LF::wrap_async_reader_with_buffer_size(test_reader, 2);
    let mut out = Vec::new();
    nr.read_to_end(&mut out).await.unwrap();
    assert_eq!(out, b"\n\n".to_vec());
});
