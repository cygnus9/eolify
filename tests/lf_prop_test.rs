use eolify::{helpers::vec_to_uninit_mut, NormalizeChunk, LF};
...
proptest! {
    ...
    fn normalize_chunk_chunk_properties(data in vec(any::<u8>(), 0..256), split_byte in any::<u8>()) {
        ...
        let status1 = LF::normalize_chunk(a, vec_to_uninit_mut(&mut out1), None, false)
        ...
        let status2 = LF::normalize_chunk(b, vec_to_uninit_mut(&mut out2), status1.state(), true)
        ...
        if !out1.is_empty() {
            for (i, &c) in out1.iter().enumerate() {
                prop_assert!(c != b'\r', "found a CR in out1 at {}", i);
            }
        } else {
        ...
        if !out2.is_empty() {
            for (i, &c) in out2.iter().enumerate() {
                prop_assert!(c != b'\r', "found a CR in out2 at {}", i);
            }
        } else {
    ...
    fn normalize_chunk_idempotent(data in vec(any::<u8>(), 0..256)) {
        ...
        let status1 = LF::normalize_chunk(&data, vec_to_uninit_mut(&mut out1), None, true)
        ...
        let status2 = LF::normalize_chunk(&out1, vec_to_uninit_mut(&mut out2), None, true)
